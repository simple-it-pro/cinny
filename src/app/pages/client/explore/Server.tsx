...
function Search({ active, loading, searchInputRef, onSearch, onReset }: SearchProps) {
  ...
  return (
    <Box as="form" direction="Column" gap="100" onSubmit={handleSearchSubmit}>
      ...
      <Text size="L400">{t('nav.search')}</Text>
      <Input
        ref={searchInputRef}
  ...
  return (
    ...
    >
      <Chip
        ...
        after={<Icon size="100" src={Icons.ChevronBottom} />}
      >
        <Text size="T200" truncate>{`${t('explore.pageLimit')}: ${limit}`}</Text>
      </Chip>
    </PopOut>
  );
}
              ...
              <Box direction="Column" gap="600">
                ...
                <Box direction="Column" gap="400">
                  <Box direction="Column" gap="300">
                    {isSearch ? (
                      <Text size="H4">{t('explore.resultsFor', { term: serverSearchParams.term })}</Text>
                    ) : (
